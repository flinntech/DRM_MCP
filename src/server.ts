...
let USER_CREDENTIALS: UserCredentialsConfig = {};
try {
  if (fs.existsSync('./credentials.json')) {
    USER_CREDENTIALS = JSON.parse(fs.readFileSync('./credentials.json', 'utf8')) as UserCredentialsConfig;
    console.error(`✓ Loaded credentials for ${Object.keys(USER_CREDENTIALS).length} users from credentials.json`);
  }
} catch (error) {
  const errorMessage = error instanceof Error ? error.message : 'Unknown error';
  console.error('⚠ Warning: Could not load credentials.json:', errorMessage);
}
// ============================================

class DigiRemoteManagerServer {
  private server: Server;
  private defaultCredentials: UserCredentials | null;
  private enabledCategories: Map<string, Set<string>>;
  private toolCategories: Record<string, ToolCategory>;
  private coreTools: string[];
  ...
  constructor() {
    ...
    this.enabledCategories = new Map<string, Set<string>>();
    this.toolCategories = {};
    this.coreTools = [];
    this.allTools = [];
...
If you encounter query syntax errors, check that you're using single quotes for all string values.`,
  ...
  private getCurrentMetadata(): RequestMetadata {
  ...
  private getAxiosClient(userId: string | null = null, requestMetadata: RequestMetadata | null = null): AxiosInstance {
  ...
  private getUserEnabledCategories(userId: string): Set<string> {
    if (!this.enabledCategories.has(userId)) {
      this.enabledCategories.set(userId, new Set<string>());
    }
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request): Promise<any> => {
      try {
        ...
        const requestMetadata = (request as any)._meta || {};

        // Store metadata in async local storage for access in all methods
        return await userContext.run({ userId: requestMetadata['X-User-Id'] as string, metadata: requestMetadata as RequestMetadata }, async () => {
          return await this.handleToolCall(name, args as Record<string, any>, requestMetadata as RequestMetadata);
        });
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
  ...
  private async handleToolCall(name: string, args: Record<string, any>, requestMetadata: RequestMetadata = {}): Promise<ToolResponse> {
    ...
    const context = userContext.getStore() || { userId: 'default' };
    ...
    try {
      switch (name) {
        ...
        if (error.response?.status === 401) {
          return {
            content: [{ type: "text" as const, text: "Authentication Error: Invalid API key." }],
            isError: false,  // Tool worked, API returned auth error
          };
        }
        if (error.response?.status === 403) {
          return {
            content: [{ type: "text" as const, text: "Permission Denied: May require Remote Manager Premier Edition." }],
            isError: false,  // Tool worked, API returned permission error
          };
        }
        if (error.response?.status === 404) {
          return {
            content: [{ type: "text" as const, text: "Not Found: Resource does not exist." }],
            isError: false,  // Tool worked, API returned not found
          };
        }
        // Only mark as isError: true for actual tool/connection failures
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        const errorData = error.response?.data ? JSON.stringify(error.response.data, null, 2) : "";
        return {
  ...
  private discoverToolCategories(): ToolResponse {
    ...
    const summary: DiscoverCategoriesResponse = {
  ...
  private buildParams(args: Record<string, any>, allowed: string[]): Record<string, any> {
  ...
  private formatResponse(data: any): ToolResponse {
  ...
  private buildSciRequest(operation: string, targets: string, payload: string): string {
...
<sci_request version="1.0">
  ...
  private buildRciRequest(command: string, content: string = ''): string {
    return `<rci_request version="1.1">
  <${command}>${content}</${command}>
</rci_request>`;
  }

  private buildDeviceTarget(deviceId: string): string {
    return `<device id="${deviceId}"/>`;
  }

  private buildMultipleTargets(targetType: string, targetValue: string): string {
    switch (targetType) {
      case 'device_ids':
        return targetValue.split(',').map((id: string) => `<device id="${id.trim()}"/>`).join('\n      ');
      case 'tag':
  ...
  private async sendSciRequest(xmlRequest: string): Promise<any> {
  ...
  private async listDevices(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async listDevicesBulk(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async getDevice(args: Record<string, any>): Promise<ToolResponse> {
    const response = await this.getAxiosClient().get(`/v1/devices/inventory/${args.device_id}`);
    return this.formatResponse(response.data);
  }

  private async listStreams(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async listStreamsBulk(args: Record<string, any>): Promise<ToolResponse> {
    const params = this.buildParams(args, ["query", "fields", "orderby"]);
    const response = await this.getAxiosClient().get("/v1/streams/bulk", { params });
    return { content: [{ type: "text", text: response.data }] };
  }

  private async getStream(args: Record<string, any>): Promise<ToolResponse> {
    const response = await this.getAxiosClient().get(`/v1/streams/inventory/${args.stream_id}`);
    return this.formatResponse(response.data);
  }

  private async getStreamHistory(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async getStreamHistoryBulk(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async getStreamRollups(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async getStreamRollupsBulk(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async getDeviceLogs(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async listGroups(args: Record<string, any>): Promise<ToolResponse> {
    const params = this.buildParams(args, ["query", "orderby"]);
    const response = await this.getAxiosClient().get("/v1/groups/inventory", { params });
    return this.formatResponse(response.data);
  }

  private async getGroup(args: Record<string, any>): Promise<ToolResponse> {
    const response = await this.getAxiosClient().get(`/v1/groups/inventory/${args.group_id}`);
    return this.formatResponse(response.data);
  }

  private async listAlerts(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async getAlert(args: Record<string, any>): Promise<ToolResponse> {
    const response = await this.getAxiosClient().get(`/v1/alerts/inventory/${args.alert_id}`);
    return this.formatResponse(response.data);
  }

  private async listMonitors(args: Record<string, any>): Promise<ToolResponse> {
    const params = this.buildParams(args, ["query", "orderby"]);
    const response = await this.getAxiosClient().get("/v1/monitors/inventory", { params });
    return this.formatResponse(response.data);
  }

  private async getMonitor(args: Record<string, any>): Promise<ToolResponse> {
    const response = await this.getAxiosClient().get(`/v1/monitors/inventory/${args.monitor_id}`);
    return this.formatResponse(response.data);
  }

  private async getMonitorHistory(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async listTemplates(args: Record<string, any>): Promise<ToolResponse> {
    const params = this.buildParams(args, ["query", "orderby"]);
    const response = await this.getAxiosClient().get("/v1/configs/inventory", { params });
    return this.formatResponse(response.data);
  }

  private async getTemplate(args: Record<string, any>): Promise<ToolResponse> {
    const response = await this.getAxiosClient().get(`/v1/configs/inventory/${args.config_id}`);
    return this.formatResponse(response.data);
  }

  private async listHealthConfigs(args: Record<string, any>): Promise<ToolResponse> {
    const params = this.buildParams(args, ["query", "orderby"]);
    const response = await this.getAxiosClient().get("/v1/health_configs/inventory", { params });
    return this.formatResponse(response.data);
  }

  private async getHealthConfig(args: Record<string, any>): Promise<ToolResponse> {
    const response = await this.getAxiosClient().get(`/v1/health_configs/inventory/${args.health_config_id}`);
    return this.formatResponse(response.data);
  }

  private async listEvents(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async listEventsBulk(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async listUsers(args: Record<string, any>): Promise<ToolResponse> {
    const params = this.buildParams(args, ["query", "orderby"]);
    const response = await this.getAxiosClient().get("/v1/users/inventory", { params });
    return this.formatResponse(response.data);
  }

  private async getUser(args: Record<string, any>): Promise<ToolResponse> {
    const response = await this.getAxiosClient().get(`/v1/users/inventory/${args.user_id}`);
    return this.formatResponse(response.data);
  }

  private async listFiles(args: Record<string, any>): Promise<ToolResponse> {
    const params = this.buildParams(args, ["query", "orderby"]);
    const response = await this.getAxiosClient().get("/v1/files/inventory", { params });
    return this.formatResponse(response.data);
  }

  private async getFile(args: Record<string, any>): Promise<ToolResponse> {
    const response = await this.getAxiosClient().get(`/v1/files/inventory/${args.file_id}`);
    return this.formatResponse(response.data);
  }

  private async getAccountInfo(_args: Record<string, any>): Promise<ToolResponse> {
    const response = await this.getAxiosClient().get("/v1/account");
    return this.formatResponse(response.data);
  }

  private async getAccountSecurity(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async getApiInfo(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async sciQueryDeviceState(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async sciQueryDeviceSettings(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async sciQueryDescriptor(args: Record<string, any>): Promise<ToolResponse> {
  ...
  private async sciQueryMultipleDevices(args: Record<string, any>): Promise<ToolResponse> {
...
<sci_request version="1.0">
  ...
  private async sciListDeviceFiles(args: Record<string, any>): Promise<ToolResponse> {
...
<sci_request version="1.0">
  ...
  private async sciGetDeviceFile(args: Record<string, any>): Promise<ToolResponse> {
...
<sci_request version="1.0">
  ...
  private async sciQueryFirmwareTargets(args: Record<string, any>): Promise<ToolResponse> {
...
<sci_request version="1.0">
  ...
  private async sciGetJobStatus(args: Record<string, any>): Promise<ToolResponse> {
    const { job_id } = args;
  ...
  private async sciGetDataServiceFile(args: Record<string, any>): Promise<ToolResponse> {
...
<sci_request version="1.0">
  ...
  private getQuerySyntaxHelpTool(): ToolResponse {
    return this.formatResponse({
      syntax_guide: this.getQuerySyntaxGuide()
    });
  }

  private getDeviceFieldsTool(): ToolResponse {
    return this.formatResponse({
      device_fields: this.getDeviceFieldsReference()
    });
  }

  private getQueryExamplesTool(args: Record<string, any>): ToolResponse {
  ...
  private validateQuerySyntaxTool(args: Record<string, any>): ToolResponse {
    const query = args.query || "";
    const validation = this.validateQuerySyntax(query);
    return this.formatResponse(validation);
  }

  private getQuerySyntaxGuide(): string {
...
- Test with simple queries first, then combine with 'and'/'or'`;
  ...
  private getDeviceFieldsReference(): string {
...
- Use list_devices without a query first to see actual field values`;
  ...
  private getQueryExamples(useCase: string): any {
  ...
  private validateQuerySyntax(query: string): any {
    const errors: any[] = [];
    const warnings: any[] = [];
  ...
  private getBuildQueryHelp(useCase: string): string {
